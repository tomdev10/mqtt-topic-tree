  ...
  Object.keys(messages).map(messageTopic => {
    ...
    arrayOfTopicLevels.map((singleNode, index) => {
      ...
      if (!checkIfArrayContainsNode(tree,idToUse,parentIdToUse)) {
        contentToUse ? tree.push({name: idToUse , displayName: messageTopic.split('/')[index], parentId: parentIdToUse, attributes: {payload: contentToUse}, loc: 1000}) : tree.push({name: idToUse , displayName: messageTopic.split('/')[index], parentId: parentIdToUse})
      }
    })
  })
...
const TreeExplorer = ({messages}) => {
  ...
  return <div style={{height: 600, border: '5px solid black', margin: '5rem'}}>
      <ResponsiveTreeMap
        ...
        value="loc"
        valueFormat=".02s"
        label={function(e){ return e.data.displayName}}
        ...
        tooltip={({ node }) => (
          <>
            ...
            <span>{node.data?.attributes?.payload ? `:${' '}${node.data?.attributes?.payload}` : null}</span>
          </>
      )}
    />
  </div>;
