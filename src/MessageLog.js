...
const NetworkExplorer = React.lazy(() =>
  import("./charts/NetworkExplorer/NetworkExplorer")
);
...
const TreeExplorer = React.lazy(() =>
  import("./charts/TreeExplorer/TreeExplorer")
);
  ...
  useEffect(() => {
    if (messages) {
      const result = messages.reduce(
        (obj, cur) => ({ ...obj, [cur.topic]: cur.message }),
        {}
      );
