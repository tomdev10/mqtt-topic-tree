...
export default function VisChart({ messages }) {
  ...
  const enrichDataForTree = (messages) => {
    Object.keys(messages).map((messageTopic) => {
      ...
      arrayOfTopicLevels.map((singleNode, index) => {
        const contentToUse =
          index === arrayOfTopicLevels.length - 1
        ...
        const parentIdToUse =
          index === 0
            ? "broker"
            : generateTopicNameFrom(messageTopic, index - 1);
        if (
          !checkIfArrayContainsNode(
            nodes,
            idToUse,
          ...
          if (contentToUse) {
            nodes.push({
              id: idToUse,
              label: messageTopic.split("/")[index],
              color: colours[index],
              shape: "box",
              mass: 3,
              margin: 10,
  ...
  return (
    <div
      className="m-2 md:m-20"
      style={{ height: 800, border: "5px solid black" }}
    >
